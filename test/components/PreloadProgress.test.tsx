...
describe('PreloadProgress', () => {
  it('does not render when isVisible is false', () => {
    render(<PreloadProgress current={0} total={10} isVisible={false}  />);

    expect(screen.queryByText('Downloading cards for offline use...')).not.toBeInTheDocument();
  });

  it('renders with text', () => {
    render(<PreloadProgress current={5} total={10} isVisible={true}/>);

    expect(screen.getByText('Downloading cards...')).toBeInTheDocument();
  });


  it('displays correct progress text', () => {
    render(<PreloadProgress current={3} total={7} isVisible={true}  />);

    expect(screen.getByText('3/7')).toBeInTheDocument();
  });

  it('calculates progress bar width correctly', () => {
    render(<PreloadProgress current={2} total={4} isVisible={true}  />);
  ...
  it('shows 0% progress when current is 0', () => {
    render(<PreloadProgress current={0} total={5} isVisible={true}  />);
  ...
  it('shows 100% progress when current equals total', () => {
    render(<PreloadProgress current={10} total={10} isVisible={true}  />);
  ...
  it('handles edge case where total is 0', () => {
    render(<PreloadProgress current={0} total={0} isVisible={true}  />);
  ...
  it('renders all required elements', () => {
    render(<PreloadProgress current={1} total={3} isVisible={true}  />);
  ...
  it('handles large numbers correctly', () => {
    render(<PreloadProgress current={500} total={1000} isVisible={true}  />);
  ...
  it('validates prop types implicitly through TypeScript', () => {
    ...
    expect(() => render(<PreloadProgress current={1} total={2} isVisible={true}  />)).not.toThrow();
  });

  it('progress bar width matches progress value', () => {
    fc.assert(
      fc.property(fc.float({ min: 0, max: 1 }), (progress) => {
        const { container } = render(<PreloadProgress current={progress * 100} total={100} isVisible={true}  />);
