...
[![Tests](https://img.shields.io/badge/tests-283_passing-brightgreen)](#testing)
...
| Area | What's inside |
|---|---|
| **Offline-first architecture** | Two-tier service worker: versioned cache-first strategy for card images (with graceful fallback to the previous cache generation) and network-first for the app shell. A user-triggered preload fetches ~300 images with a live progress bar, and a post-build script injects hashed asset names into the service worker so every deploy caches correctly. |
| **Privacy by design** | Analytics and crash reporting are separate opt-ins, re-verified on every tracking call and disabled offline by default. Playwright tests intercept network traffic and assert that *zero* requests are sent when consent is declined. |
| **Testing culture** | 283 tests: 242 unit/integration tests (Vitest) and 41 end-to-end tests (Playwright) across Chromium, Firefox, WebKit, Mobile Chrome, and Mobile Safari — roughly 2.7 lines of test code for every line of application code. Includes fast-check property-based tests for deck/shuffle invariants, a custom flakiness harness that runs the suite 10× to catch instability, and CSS parser tests that lock touch/hover behavior in as executable contracts. |
...
- **Consent-gated telemetry** — Analytics (Umami, auto-track disabled) and crash reporting each require explicit opt-in; error reports are POSTed to a separate microservice only with consent.
...
- **Unit/integration (Vitest):** 242 tests across 16 files — deck construction invariants, shuffle correctness, consent and tracking logic, error reporting lifecycle, service worker registration, settings/menu components, and Express API tests via supertest.
...
- **End-to-end (Playwright):** 41 tests in 4 suites run across 5 browser projects — consent matrix with network interception, keyboard/click/touch flip behavior (asserted on computed transforms), responsive layout at 375/768/1920 px, offline mode, corrupted localStorage recovery, and image 404 handling.
...
- **Error receiver** (`error-receiver/`) — a minimal Express service at `errors.cards.unimpossy.com` that accepts consent-gated crash reports and logs them as structured JSON for collection.
...
```
app/
  routes/          # Home (card flipper) and card-lists (catalog) routes
  card/            # 3D flip card component
  components/      # Settings, consent popup, menus, progress bar
  utils/           # Deck building, shuffling, error reporting
public/
  cards/           # 330 card images (front/back pairs)
  service-worker.js
